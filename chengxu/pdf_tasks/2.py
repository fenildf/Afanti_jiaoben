...
logging.basicConfig(format=LOGGING_FORMAT, level=logging.INFO,
                    filename='working/2.log', filemode='a')
...
def pdf(src,dst):
    try:
        if not os.path.exists(dst):
            options = {'quiet': ''}
            pdfkit.from_file(src,dst,configuration=config,options=options)
        else:
...
def gendir():
    try:
        for sb in subjects.items():
            subject,sbname=sb
            os.makedirs(INPUT_PATH + 'pdf_{0}/{1}'.format(time.strftime("%y%m%d"), sbname))
            os.makedirs(OUT_PATH + 'pdf_{0}/{1}'.format(time.strftime("%y%m%d"),sbname))
    except Exception as e:
        pass


def dealMark(mark_pdf_list):
    f_tupain = 'afanti.png'
...
if __name__ == '__main__':
    # gendir()
    mark_pdf_list = ['mark.pdf','mark1.pdf']
    dealMark(mark_pdf_list)
    for root,dirs,files in os.walk('export_paper_20180307'):
        for f in files:
            src = os.path.join(root, f)
            if '.DS_Store' in src:
                continue
            name = f.replace('html', 'pdf').replace('answer', '答案')
            subject = root.replace('\\','/').split('/')[1]
            dst = os.path.join(INPUT_PATH, 'pdf_{}'.format(time.strftime("%y%m%d")),subject,name)
            print(src, '->', dst)
            pdf(src, dst)
            add_watermark(
                pdf_file_in=dst,
                mark_list=mark_pdf_list,
                pdf_file_out=dst.replace(INPUT_PATH, OUT_PATH)
            )
